...
# Change: the reweighted-L1 second solve is disabled (commented out, in
# `update_c_parallel!`). Upstream applies
#     lambda_l1 = l1_coeff / (1 + 200 * abs(solution))
# and re-solves, to correct L1 shrinkage bias. That path did not work on the
# behavioral data used here, so every result in the paper comes from the
# non-reweighted version. Confirmed acceptable by the upstream author.
# Also adds per-iteration gradient-norm diagnostics.
...
# A Jacobi update would allow parallelization across time.
"""
...
function update_F!(
    ...
    # Gradient magnitude (for troubleshooting)
    g_norm = norm(gradient_sum)
    g_max = maximum(abs.(gradient_sum))
    scale = lr_F / (num_timepoints * size(c, 1))
    println("gradient_sum: norm=$g_norm, max_abs=$g_max, step_norm=$(scale * g_norm)")

    # Take the step
    F .+= scale .* gradient_sum
    ...
    g_norm_decorr = norm(gradient_sum)
    g_max_decorr = maximum(abs.(gradient_sum))
    println("gradient_sum after decorr: norm=$g_norm_decorr, max_abs=$g_max_decorr")
